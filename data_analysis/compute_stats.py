...
df_commits = pd.read_json("output/commits.jsonl", lines=True)
all_commits = df_commits["commit_id"].unique().tolist()

# read the JSONL file into a pandas DataFrame
df = pd.read_json("output/data.jsonl", lines=True)

# read the JSONL file into a pandas DataFrame
df_filelang = pd.read_json("output/language_data.jsonl", lines=True)
...
print(interesting_commits[4:6][["commit_id", "total", "message"]].to_string())
exit(0)
